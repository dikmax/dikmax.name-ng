...
-- Original repository https://github.com/bgamari/pandoc-lens/
...
_Link :: Prism' Inline (Attr, [Inline], Target)
_Link = prism' (\(a, b, c) -> Link a b c) f
  where
    f (Link a b c) = Just (a, b, c)
    f _            = Nothing

-- | A prism on a 'Image' 'Inline'
_Image :: Prism' Inline (Attr, [Inline], Target)
_Image = prism' (\(a, b, c) -> Image a b c) f
  where
    f (Image a b c) = Just (a, b, c)
    f _             = Nothing
...
instance HasAttr Inline where
    ...
    attributes f (Image a i s) = fmap (\a'->Image a' i s) (f a)
    attributes f (Link a i s)  = fmap (\a'->Link a' i s) (f a)
